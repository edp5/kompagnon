...
import { findAcceptedFoundJourneysByJourneyId } from "../../../../src/journeys/repositories/found-journeys-repository.js";
...
describe("Unit | Journeys | UseCases | getTrackingPointsUsecase", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("should throw UserHasNoRole if user has no role", async () => {
    ...
    await expect(getTrackingPointsUsecase({ userId: 1, journeyId: 1 })).rejects.toThrow(UserHasNoRole);
  });
  ...
  it("should throw JourneyIsNotOfThisUser if user is neither owner nor match partner", async () => {
  ...
  it("should return tracking points for journey owner (passenger)", async () => {
    findUserById.mockResolvedValue({ role: USER_ROLE.PASSENGER });
  ...
  it("should return tracking points for confirmed match partner", async () => {
    findUserById.mockResolvedValue({ role: USER_ROLE.COMPANION });
    findCompanionJourneyById.mockResolvedValue({ userId: 99 });
    findAcceptedFoundJourneysByJourneyId.mockResolvedValue([{ userId: 1 }]);
    const points = [{ id: 2, lat: 48.5, lon: 2.3 }];
    findTrackingPointsByJourneyId.mockResolvedValue(points);
  ...
  it("should return tracking points for journey owner (companion)", async () => {
    findUserById.mockResolvedValue({ role: USER_ROLE.COMPANION });
